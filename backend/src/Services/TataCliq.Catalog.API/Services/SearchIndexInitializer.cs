 ...
 *   1. Create or update the "tatacliq-products" search index with correct schema
 *   2. Create or update the "tatacliq-synonyms" synonym map from DB SearchSynonyms table
    ...
    ILogger<SearchIndexInitializer>         logger) : BackgroundService
{
    ...
    private async Task ProvisionIndexAsync(
        ...
        CancellationToken ct)
    {
        var indexName      = configuration["AzureCognitiveSearch:IndexName"] ?? "tatacliq-products";
        var synonymMapName = configuration["AzureCognitiveSearch:SynonymMapName"] ?? "tatacliq-synonyms";
