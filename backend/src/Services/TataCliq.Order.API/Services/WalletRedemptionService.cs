/**
 * ENH-PAY-007 — Wallet CLiQ Cash Redemption with pessimistic SELECT…WITH (UPDLOCK, ROWLOCK)
 * Acceptance criteria:
 ...
 *   - Structured log events: CLIQ_CASH_REDEEMED, CLIQ_CASH_REDEMPTION_IDEMPOTENT
 */
...
/// <summary>
/// ENH-PAY-007 — Thrown when the user's CLiQ Cash balance is insufficient
...
public sealed class InsufficientWalletBalanceException(decimal requested, decimal available)
    : Exception($"Insufficient CLiQ Cash balance: requested ₹{requested:N2}, available ₹{available:N2}")
{
...
{
    /// <summary>
    /// ENH-PAY-007 — Debits CLiQ Cash from the user's wallet inside a pessimistic-locked
    ...
    /// <summary>Returns the current CLiQ Cash balance. Returns 0 when no wallet exists.</summary>
...
/// <summary>
/// ENH-PAY-007 — Wallet CLiQ Cash debit with pessimistic UPDLOCK/ROWLOCK concurrency guard.
        ...
        Guid userId, decimal amount, string orderNumber, CancellationToken ct)
    {
        ...
        if (alreadyRedeemed)
        {
            logger.LogInformation(
                ...
                "CLIQ_CASH_REDEMPTION_IDEMPOTENT", userId, orderNumber);
            return;
        }
        ...
        db.WalletTransactions.Add(new WalletTransaction
        {
            ...
            Description  = $"CLiQ Cash redeemed on order {orderNumber}",
        ...
        logger.LogInformation(
            ...
            "CLIQ_CASH_REDEEMED", userId, orderNumber, amount, wallet.Balance);
    }
