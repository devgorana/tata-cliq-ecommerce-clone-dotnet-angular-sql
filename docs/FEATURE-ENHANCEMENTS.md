...
|---|---|---|---|---|---|---|---|
| ENH-AUTH-001 | Facebook OAuth 2.0 Login | FR-AUTH-004 | P1 | P1 | [ ] | YES | BOTH |
| ENH-AUTH-002 | Apple Sign-In (hidden-email proxy) | FR-AUTH-005 | P1 | P1 | [ ] | YES | BOTH |
| ENH-AUTH-003 | Account Merge — social email matches verified account | FR-AUTH-009 / BR-AUTH-007 | P1 | P0 | [ ] | YES | BOTH |
| ENH-AUTH-004 | Multi-device Session Management (view + remote-logout) | FR-AUTH-008 | P1 | P0 | [ ] | YES | BOTH |
| ENH-AUTH-005 | Account Lockout Exponential Doubling (30min → doubles, max 24h) | FR-AUTH-011 / BR-AUTH-008 | P1 | P0 | [ ] | YES | BOTH |
...
| ENH-CAT-002 | Flash Sale Module — server-driven countdown, sold-out transition | FR-HOME-005 | P5 | P0 | [ ] | YES | BOTH |
...
| ENH-ID | Feature Title | Source | Phase | Priority | Status | Parallel-testable | Agent |
|---|---|---|---|---|---|---|---|
| ENH-PDP-001 | Pincode Delivery Estimate — serviceability, COD eligibility, ETA ≤1s | FR-PDP-003 | P2 | P0 | [ ] | YES | BOTH |
| ENH-PDP-002 | EMI Calculator — bank-wise tenure, no-cost EMI in accent-red, min-order config | FR-PDP-004 / BR-PDP-003 | P2 | P0 | [ ] | YES | BOTH |
...
| ENH-SRCH-002 | Search Autocomplete + Typeahead (≤200ms p95) | FR-SRCH (TSD §7.1) | P2 | P0 | [ ] | YES | BOTH |
| ENH-SRCH-003 | Search Synonyms Dictionary managed via Admin CMS | FR-SRCH (TSD §7.1) | P4 | P1 | [ ] | YES | BOTH |
...
| ENH-ID | Feature Title | Source | Phase | Priority | Status | Parallel-testable | Agent |
|---|---|---|---|---|---|---|---|
| ENH-CART-001 | Optimistic UI with NgRx rollback on HTTP 4xx/5xx (toast within 500ms) | FR-CART-003 | P2 | P0 | [ ] | YES | BOTH |
...
| ENH-ID | Feature Title | Source | Phase | Priority | Status | Parallel-testable | Agent |
|---|---|---|---|---|---|---|---|
| ENH-CHKOUT-001 | Email Verification Gate for Checkout > ₹5,000 | BR-AUTH-003 | P3 | P0 | [ ] | YES | BOTH |
| ENH-CHKOUT-002 | Express Checkout — one-tap with saved address + saved payment | FR-CHKOUT (TSD §5) | P3 | P1 | [ ] | YES | BOTH |
| ENH-CHKOUT-003 | COD Ceiling Enforcement (₹50,000 max) | TC-PAY-BVA-002 / SOW §3.8 | P3 | P0 | [ ] | YES | BOTH |
...
| ENH-PAY-002 | HMAC-SHA256 Webhook Verification — constant-time compare, mismatch → HTTP 401 + audit | FR-PAY-009 / TE-008 (implicit) | P3 | P0 | [ ] | YES | BOTH |
...
| ENH-PAY-007 | Wallet CLiQ Cash Redemption — pessimistic lock SELECT…WITH (UPDLOCK, ROWLOCK) | FR-PROMO-005 / TC-CART-FUNC-022 | P5 | P0 | [ ] | YES | BOTH |
...
| ENH-ID | Feature Title | Source | Phase | Priority | Status | Parallel-testable | Agent |
|---|---|---|---|---|---|---|---|
| ENH-ORD-001 | Order State Machine CHECK Constraints on OrderStatusHistory (valid enum only) | FR-ORD-002 / TE-006 | P3 | P0 | [ ] | YES | BOTH |
| ENH-ORD-002 | Concurrent State Transition Protection (HTTP 409 ORDER_STATE_CONFLICT) | FR-ORD-002 / TC-ORD-FUNC-036 | P3 | P0 | [ ] | YES | BOTH |
...
| ENH-ID | Feature Title | Source | Phase | Priority | Status | Parallel-testable | Agent |
|---|---|---|---|---|---|---|---|
| ENH-PROMO-001 | CLiQ Cash Earn on Purchase (configurable % of order value) | FR-PROMO (SOW §3.10) | P5 | P0 | [ ] | YES | BOTH |
| ENH-PROMO-002 | CLiQ Cash Expiry Policy (12-month inactivity) | FR-PROMO (SOW §3.10) | P5 | P1 | [ ] | YES | BOTH |
| ENH-PROMO-003 | Flash Sale Price Lock — server-driven, race-condition-safe | FR-HOME-005 / EC-INV | P5 | P0 | [ ] | YES | BOTH |
| ENH-PROMO-004 | Coupon Stacking Rules — configurable allow/deny per coupon type | FR-CART-006 (g) | P5 | P1 | [ ] | YES | BOTH |
...
| ENH-ID | Feature Title | Source | Phase | Priority | Status | Parallel-testable | Agent |
|---|---|---|---|---|---|---|---|
| ENH-NOTIF-001 | Exponential Backoff Retry — 1m, 3m, 9m then Azure Service Bus DLQ (7-day retention) | FR-NOTIF-006 | P4 | P0 | [ ] | YES | BOTH |
| ENH-NOTIF-002 | FCM Push Notifications (order updates, flash sale, back-in-stock) | SOW §2.1 | P4 | P0 | [ ] | YES | BOTH |
| ENH-NOTIF-003 | WhatsApp Business Notification Channel (MSG91 WhatsApp) | SOW §2.1 | P4 | P1 | [ ] | NO | IMPL |
| ENH-NOTIF-004 | Email OTP via Hangfire Job + MailKit / Azure Communication Services | Phase 9.6 deferred item | P1 | P0 | [ ] | YES | BOTH |
...
| ENH-ID | Feature Title | Source | Phase | Priority | Status | Parallel-testable | Agent |
|---|---|---|---|---|---|---|---|
| ENH-ADMIN-001 | AuditLogs Schema — append-only, 7y financial / 3y non-financial retention policy | FR-ADMIN-008 | P4 | P0 | [ ] | YES | BOTH |
| ENH-ADMIN-002 | Hangfire Dashboard (admin-only route) + Job Management UI | Phase 9.8 deferred | P4 | P1 | [ ] | NO | IMPL |
| ENH-ADMIN-003 | Scheduled Jobs: DailyAnalyticsJob, LowStockAlertJob, CartAbandonmentJob, ExpireCouponsJob | Phase 9.8 deferred | P4 | P1 | [ ] | YES | BOTH |
...
| ENH-ID | Feature Title | Source | Phase | Priority | Status | Parallel-testable | Agent |
|---|---|---|---|---|---|---|---|
| ENH-SELL-001 | Multi-Tenant Row-Level Security (SQL Server RLS on SellerProducts) | TSD §6.1 / AG-002 | P4 | P0 | [ ] | YES | BOTH |
...
| ENH-INFRA-006 | Blue-Green Deployment with Auto-Rollback (error rate >1% → slot swap back) | FR-OPS-004 | P7 | P0 | [ ] | YES | BOTH |
...
| ENH-INFRA-008 | k6 Load Tests — Spike (100→10K in 30s) + Soak (1K × 60min) | TC-NFR-LOAD-003/004 / NFR-PERF-008 | P6 | P0 | [ ] | YES | TEST |
| ENH-INFRA-009 | OWASP ZAP Automated Scan in CI Pipeline (fail on Critical/High) | TC-OPS-SEC-001..003 / NFR-SEC-A01..A09 | P6 | P0 | [ ] | YES | TEST |
